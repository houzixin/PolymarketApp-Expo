...
class MatcherService {
   ...
   * 匹配 outcomes
   */
  match(polymarketOutcomes, kalshiOutcomes, threshold = 0.7) {
    const matches = [];
    ...
    for (const polyOutcome of polymarketOutcomes) {
      ...
      for (let i = 0; i < kalshiOutcomes.length; i++) {
        ...
        const score = this.combinedSimilarity(polyOutcome.title, kalshiOutcomes[i].title);
